...
export class ProductPage {
	...
	ionViewDidLoad(){
		...
		// console.log(333333,new Date("2017-08-11 12:00:00".replace(/-/g, "/")))
		// console.log(Utils.dateFormat(new Date("2017/08/11 12:00:00"),'yyyy/MM/dd HH:mm:ss'))
		//交易socket
		...
		this.globalData.socketdata.forEach((value,index,arr)=>{
			if(value.instrumentId==this.code+this.contract){
				this.socketdata=value;
				this.quote=Number(value.nowPrice)-Number(value.preClosePrice);
				this.quoteRate=this.quote/Number(value.preClosePrice)*100;
			}
		})
		this.events.subscribe('quotesSocketData:product',(msg)=>{
			...
			if(tempdata.code=='00'){
				...
				tempdata.data.tradeTimes.forEach((value1,index1,arr1)=>{
					if(this.code==value1.futuresCode){
			...
			if(res.success=="true"){
				...
				if(this.tab!='k1'){//非分时线
					...
					if(this.data.length<40){//当小于40跟k线时，将数据增添至40跟；
						...
						console.log('111111',this.start,this.end)
				...
				setTimeout(()=>{
					...
					if(this.tab=='k1'){
						...
						if(this.data.length<40){//当小于40跟k线时，将数据增添至40跟；
							...
							this.myChart.on('dataZoom', this.changePosition)
							this.myChart.setOption(drawconfig._candle(this.data,Number(localStorage.start),Number(localStorage.end)));
						}
					}
