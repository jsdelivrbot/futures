...
export class PurchasePage {
	...
	private choose2:number=1;//止损选择项--UI效果控制
	private choose2_step:number=1;//止损挡位倍数
	private choose2_val:number;//止损选择项--金额数
	private choose3:number=1;//止盈选择项--UI效果控制
	private choose3_val:number;//止盈选择项--金额数
	...
	ionViewDidLoad(){
		this.events.subscribe('goNextPage:OrderPage',()=>{
			this.navCtrl.push(OrderPage)
		});
		...
		this.events.subscribe('buyUpOrDown',(val)=>{
			...
			let orderDate= this.OrderInfo.create({
				...
				StopLossMoney:this.choose2_val/this.choose1,//止损金额 ,比如1400，就代表该笔交易亏1400元人民币就平仓， 0代表没有设置
				StopProfitMoney:this.choose3_val/this.choose1,//止盈金额，比如2000，就代表该笔交易赚2000元人民币就平仓， 0代表没有设置
	...
	getInital(){
		...
		.subscribe(res => {
			...
			if(res.success=="true"){
				...
				this.choose3_val=this.deposit*this.stoplossPercent*this.choose1*this.stopprofitMutipleLoss;
				this.totalFee=this.feeOpen*this.choose1;//总手续费计算操作
	...
	//选择止盈挡位
	secChoose1(val1,val2){
		this.choose3=val1;
		this.choose3_val=val2;
		// switch(val1){
		// 	case 1:
		// 		this.choose2_step=1;
		// 		break;
	...
	butChange(){
		this.totalFee=this.feeOpen*this.choose1;//总手续费计算操作计算优惠券之前
